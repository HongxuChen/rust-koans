...
#[test]
fn vec_macro() {
    let vector: vec::Vec<u32> = vec![1, 2, 3, 4];
    assert!(vector.len() == 4);
}
...
#[test]
fn adding_to_vecs() {
    let mut vector: vec::Vec<u32> = vec![0, 1, 2];
    vector.push(3);
    assert!(vector.len() == 4);
}
...
fn removing_from_end_of_vecs() {
    let mut vector: vec::Vec<u32> = vec![0, 1, 2, 3];
    vector.pop();
    assert!(vector.len() == 3);
}
...
fn changing_size_of_vecs() {
    let mut vector: vec::Vec<u32> = vec![1, 2, 3];
    vector.resize(4, 0);
    assert!(vector.len() == 4);
    vector.resize(3, 0);
    assert!(vector.len() == 3);
...
#[test]
fn shrink_vecs() {
    let mut vector: vec::Vec<u32> = vec![1, 2, 3, 4, 5];
...
#[test]
fn reserve() {
    let mut vector: vec::Vec<u32> = vec![1];
    vector.reserve(7);
    assert_eq!(vector.capacity(), 8);
}
...
#[test]
fn empty_vecs() {
    let vector: vec::Vec<&'static str> = vec![""; 0];
...
#[test]
fn vec_indices() {
    let vector: vec::Vec<&'static str> = vec!["red", "green", "refactor"];
...
#[test]
fn first_and_last() {
    let vector: vec::Vec<bool> = vec![false, true];
...
fn contains_element() {
    ...
    assert!(!vector.contains(&"Facebook"));
}
...
fn starts_with_2() {
    let vector = vec![0, 2, 4, 6];
    assert!(vector.starts_with(&vec![0, 2, 4]));
}
...
#[test]
fn splitting() {
    let vector: vec::Vec<&'static str> = vec!["Ruby", "Rust", "Python", "C++"];
...
// note: this comes from slice::split
#[test]
fn more_splitting() {
    let vector: vec::Vec<u32> = vec![1, 3, 4, 7, 9];
    // the match one is NOT contained in subslices
    for num in vector.split(|&x: &u32| x % 4 == 0) {
